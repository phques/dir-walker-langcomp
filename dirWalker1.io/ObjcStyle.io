...
//~ 	self
//~ )
//## pq, dont keep ':'s, so we can use objc like syntax in *Io*
//# nb defining: toto:with: := method(p1,p2..) does NOT work (cant get it to anyways ;-p)
Message unInfix := method(l,
	if(self name endsWithSeq(":"),
        self setName(self name exclusiveSlice(0, -1))
		while(next and next name endsWithSeq(":"),
			self setName(self name .. next name exclusiveSlice(0, -1))
			self setArguments(self arguments appendSeq(next arguments))
			self setNext(next next)
		)
	)
